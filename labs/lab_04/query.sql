...
returns int as
    $$
    query = plpy.prepare("select count(*) as cnt from payments JOIN content c on c.id = payments.content_id JOIN creators cr on c.id = cr.id where cr.id = $1", ["int"])
    res = plpy.execute(query, [creator_id])
    return res[0]['cnt']
    $$ language plpython3u;
select * from get_count_donators(1);
...
select * from get_sum_max_donations()
